...
Iegor's code
```{r}
item_full_info <- read_html("https://www.immoscout24.ch/en/real-estate/rent/city-lugano?pn=5") %>% 
  html_nodes(".dAzzeo") %>%
  html_text()
...
i <- 1
for (i in length(item_full_info)){
  print(str_split(item_full_info, pattern=",")[[i]][length(str_split(item_full_info, pattern=",")[[i]])-2])
}

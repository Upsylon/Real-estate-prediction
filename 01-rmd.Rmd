...
Before beginning anything, we have to be sure that we are respecting the utilization conditions of Immoscout24.  
Therefore, we look if the path to which we will webscrapp the data is allowed by the website.
...
The first step of our process is to webscrapp the data from Immoscout24.  
...
This function takes as an argument a vector containing the names of the cities for which the user would like to webscrap the data and returns a tidy dataframe with all relevant informations.  
...
It is possible to click on the different points or to brush many at the same time to retrieve their informations.  
...
This function is an amelioration of our first *get_immodata* function and directly provides all the information webscrapped, the longitude, the latitude and the predicted price.  

The following dataframe is the result of this function used on the 8 biggest cities of Switzerland. 


```{r}
# result from get_immodata2(...) 
all_cities <- readRDS(file = "all_cities.rds") 

# result from get_immodata2(...) %>% predict_price() %>% summary()
all_cities2 <- readRDS(file = "all_cities2.rds") 
```
